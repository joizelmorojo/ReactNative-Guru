...
import { TeamChallengeEndChallenge } from "../../modules/actions";
import { ChallengeQuestionStyles } from "./ChallengeQuestionTeamStyle";
...
function ChallengeQuestionPageTeam(props) {
  ...
  useEffect(() => {
    let maxTime = props.challengeObj.parameters[1].value;
    setMaxTime(maxTime);
    setStop(false);
    setShowFab(true);
    setAnswer(false);
  ...
  const checkAnswer = (obj, i) => {
    if (!stop) {
      ...
      setStop(true);
      setShowFab(false);
      let challengeParams = props.challengeObj.parameters;
      let points = obj.iscorrect ? Number(challengeParams[0].value) : 0;
      let answerTimee = Number(props.challengeObj.parameters[1].value) - maxTime;
  ...
  const next = (index, questions, obj) => {
    ...
    if (indexx < questions.length) {
      ...
      setStop(false);
      setShowFab(true);
      setMaxTime(props.challengeObj.parameters[1].value);
      setIndex(indexx);
      setAnswer("");
      setAnswerTime(0);
      let statee = state;
      ...
      props.TeamChallengeEndChallenge(data);
    }
  };
